% SSD Cost Function Visualization with GRBF and Translation Estimation
% This script computes the Sum of Squared Differences (SSD) cost surface 
% using Gaussian Radial Basis Functions (GRBF) for image representation. 
% It iterates over a range of translation parameters, evaluates the cost 
% function, and visualizes the 3D cost surface with marked true and estimated 
% translations for a given sigma.
...
for sigma_idx = 1:length(sigma_values)
    ...
    for i = 1:length(theta_x_range) % Loop over theta_x
        for j = 1:length(theta_y_range) % Loop over theta_y
            ...
            term1 = sum_GRBF(I_c.^2, delta, translation_vector, sigma, 2);
            term2 = sum_GRBF(I_c, delta, translation_vector, sigma, 2) .* I_d_GRBF;

            % Store cost in the corresponding grid location
            cost_grid(j, i) = 0.5 * sum(term1(:)) - sum(term2(:)) + ...
